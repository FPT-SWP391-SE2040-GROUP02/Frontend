...
const mockAssetsStore: AssetItemDto[] = [
  {
    id: "ast_01",
    vaultId: "vlt_01",
...
export async function getAssets(
  ...
  try {
    const response = await axiosClient.get<ApiResponse<PaginatedList<AssetItemDto>>>(
      ASSETS_ENDPOINT,
      { params }
    );
    ...
    console.warn("[assetService] getAssets fallback:", err);
  }

  return {
    items: mockAssetsStore,
    totalCount: mockAssetsStore.length,
...
export async function getAssetDetail(id: string): Promise<AssetItemDto> {
  try {
    const response = await axiosClient.get<ApiResponse<AssetItemDto>>(
      `${ASSETS_ENDPOINT}/${id}`
    );
    if (response.data?.data) {
      return response.data.data;
    ...
    console.warn(`[assetService] getAssetDetail fallback for ${id}:`, err);
  }
...
export async function createAsset(
  ...
  try {
    const response = await axiosClient.post<ApiResponse<AssetItemDto>>(
      ASSETS_ENDPOINT,
      payload
    );
    if (response.data) {
    ...
    console.warn("[assetService] createAsset fallback:", err);
  }
...
export async function deleteAsset(id: string): Promise<ApiResponse<null>> {
  try {
    ...
    console.warn(`[assetService] deleteAsset fallback for ${id}:`, err);
  }
...
export async function getVaultStats(): Promise<AssetVaultStats> {
  try {
    const response = await axiosClient.get<ApiResponse<AssetVaultStats>>(
      `${ASSETS_ENDPOINT}/stats`
    );
    if (response.data?.data) {
      return response.data.data;
    }
  } catch (err) {
    console.warn("[assetService] getVaultStats fallback:", err);
  }
